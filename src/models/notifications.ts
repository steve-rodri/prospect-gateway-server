...
export const createNotification = async (
  ...
  const getTypeIdQuery = dbc("notification_types")
    .where("name", "=", notificationName)
    .select("id");
  const getSenderIdQuery = dbc("users")
    .where("username", "=", senderName)
    .select("id");
  const getRecipientIdQuery = dbc("users")
  ...
  const locateDuplicateQuery = dbc("notifications")
    .where("key", "=", key)
